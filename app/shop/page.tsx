                ...
                >
                  <div style={{ position: 'relative', overflow: 'hidden' }}>
                    <div style={{ width: '100%', aspectRatio: '4/3', background: '#e8e8ec', overflow: 'hidden' }}>
                      <img src={product.img} alt={product.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                    </div>
                    <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.3) 0%, transparent 50%)' }} />
                    {product.badge && (
                      <div style={{ position: 'absolute', top: 12, left: 12, background: product.badgeColor, color: '#fff', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                        {product.badge}
                      </div>
                    )}
                  </div>
                  <div style={{ padding: '20px 20px 24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <span style={{ fontSize: 11, fontWeight: 600, color: '#7B1A2D', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>{product.cat}</span>
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: '#1A1A2A', lineHeight: 1.4, marginBottom: 8, flex: 1 }}>{product.title}</h3>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto', paddingTop: 16, borderTop: '1px solid rgba(0,0,0,0.06)' }}>
                      ...
                      <button style={{ padding: '8px 18px', borderRadius: 30, fontSize: 13, fontWeight: 600, background: '#E5A52E', color: '#fff', border: 'none', cursor: 'pointer', transition: 'all 0.2s' }}
